...
const App: React.FC = () => {
                ...
                try {
                  ...
                  if (parsed.chunk) {
                    ...
                    setNotes((prev) =>
                      prev.map((note) =>
                        note.id === newLineId
                          ? { ...note, translation: accumulated, isTranslating: true }
                          : note
                      )
                    );
                  }
                } catch {
                  // Silent ignore invalid JSON chunks
                }
              }
            }
          }
          ...
          setNotes((prev) =>
            prev.map((note) =>
              note.id === newLineId
                ? {
                    ...note,
                    translation: accumulated || 'No translation received',
                    isTranslating: false,
                  }
                : note
            )
  ...
  const exportNotes = (format: 'txt' | 'pdf' | 'doc') => {
    ...
    if (format === 'txt') {
      ...
      const content = notes
        .map((n) => `<p><b>${n.original}</b> — <i>${n.translation || '...'}</i></p>`)
        .join('');
      const html = `
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-800">
      ...
      <footer className="p-4 text-center text-slate-400 text-xs">
        © 2024 LingoNotes AI • Your notes are stored locally on this device.
      </footer>
    </div>
  );
};
